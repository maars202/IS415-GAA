...
In this section, you will learn how to compute the kernel density estimation (KDE) of grab trips in Singapore.
...
The code chunk below is used to plot these four study areas and the locations of the grab trip destinations.
...
as we can the lower bottom part of Pungol is heavily saturated with driving trip destination end points.
...
The Projected CRS for the road_network above is SVY21 / Singapore TM due to the st_transform method we applied earlier when importing the data. Now, lets extract the geometry data and make it to a spatial object which can be converted to a psp object to be used for fitting inside Singapore's boundary window.
...
```{r}
#road_network_sp <- as(road_network_sp_lines, "SpatialPoints")
#road_network_sp
```

since ppp is only used for SpatialPoints, psp is used for line segments and we convert it to that for windowing later

```{r}
#road_network_ppp <- as(road_network_sp, "ppp")
#road_network_ppp
```

```{r}
#road_network_psp <- as(road_network_sp, "psp")
#road_network_psp
```

```{r}
#road_network_psp = road_network_psp[sg_owin]
```

```{r}
#plot(road_network_psp)
```
...
Using multicore support for this function as there are alot of roads in Spatiallines object that need to be cut into specified minimal distance.
...
![](http://bboxfinder.com/#0.802113,99.660412,7.514393,119.260099){fig-align="left"}
...
103.54,1.159704,104.100274,1.515345
