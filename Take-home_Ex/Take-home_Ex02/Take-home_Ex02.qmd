...
```{r}
#dengue_df_count$VILLNAME  = replace(dengue_df_count$VILLNAME, "None", NA)
#colSums(is.na(dengue_df_count))
dengue_df_count$VILLNAME[dengue_df_count$VILLNAME == 'None'] <- ''
dengue_df_count
```

```{r}
taiwan_sf_filtered$VILLNAME[taiwan_sf_filtered$VILLNAME == 'None'] <- ''
taiwan_sf_filtered
```
...
```{r}
print(table(dengue_df_combined$VILLNAME[dengue_df_combined$VILLNAME == 'None']))
print(length(table(dengue_df_combined$VILLNAME)))
...
total_invalids = 0
for(i in 1:nrow(dengue_df_distribution)+1)
{
...
shortlisted_colors <- subset(dengue_df_combined, VILLTOWN %in% dengue_df_distribution2$VILLTOWN)
shortlisted_colors
```
...
```{r}
is_na(dengue_df_combined)
```

```{r}
bos = spacetime(shortlisted_colors, taiwan_sf_filtered,
                .loc_col = "VILLTOWN",
