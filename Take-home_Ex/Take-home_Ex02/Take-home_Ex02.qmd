...
```{r}
taiwan_s <- as_Spatial(taiwan_sf_filtered)
...
```{r}
dengue_df_spatial_object <- as_Spatial(dengue_df_filtered)
```

```{r}
dengue_sp <- as(dengue_df_spatial_object, "SpatialPoints")
dengue_sp
```
...
```{r}
#plotting to be done on AWS EC2 due to low computing power
#tmap_mode('view')
...
```{r}
dengueTAIWAN_ppp = dengue_ppp[taiwan_owin]
```
